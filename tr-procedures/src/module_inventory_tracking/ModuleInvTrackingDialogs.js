...
export function ModuleInvTrackingDialogs(base) {
  return class extends base {
    ...
    cleanLotPrintLabelDialogFields() {
      //this.instrumentInput.value = "";
      //this.instrumentFamilyInput.value = ""
    }
    newInvTrackingTemplate() {
      return html`
      <tr-dialog id="lotPrintLabelDialog" 
        ?open=${this.familyList.length}
        @closed=${e => { if (e.target === this.lotPrintLabelDialog) this.cleanLotPrintLabelDialogFields() }}
        heading=""
        hideActions=""
        scrimClickAction="">
    ...
    get lotPrintLabelDialog() {
      return this.shadowRoot.querySelector("tr-dialog#lotPrintLabelDialog")
    }
    ...
    instrumentEventTemplate() {
      ...
      html``: html`
        ...
        @closed=${() => this.cleanLotPrintLabelDialogFields()}
        heading=""
        hideActions=""
        scrimClickAction="">
          <div class="layout vertical flex center-justified">
    ...
    newInstrumentAction() {
      ...
      if (this.selectedAction.endPoint){
        ...
        params = params + this.config.ApiInvTrackingAPIactionsUrl
      }
      params = params + '?' + new URLSearchParams(this.reqParams)
      this.fetchApi(params).then(() => {
        this.lotPrintLabelDialog.close()
        this.reload()
      })
    }    
    ...
    completeInstrumentEventAction() {
      ...
      //let params = this.config.backendUrl + this.config.ApiInvTrackingAPIactionsUrl
